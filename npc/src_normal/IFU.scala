...
class IFU extends Module {
  val io = IO(new Bundle {
    ...
    // ── BTB interface ──
    val btb_hit    = Input(Bool())       // BTB 查询命中
    val btb_target = Input(UInt(32.W))   // BTB 预测的目标地址
  })
    ...
    // 优先级: flush(最高) > BTB命中 > 顺序执行
    when(io.flush_valid) {
        pre_pc := io.flush_pc
    } .elsewhen(io.btb_hit && io.if_sram.req_valid && io.if_sram.addr_ok) {
        pre_pc := io.btb_target
    ...
    val btb_hit_reg = RegInit(false.B)      // 记录这条指令是否被 BTB 预测
    when(io.if_sram.req_valid && io.if_sram.addr_ok && !io.flush_valid) {
        pc_reg := io.if_sram.addr
        btb_hit_reg := io.btb_hit          // 锁存 BTB 查询结果
    }
    ...
    io.out.bits.btb_hit := btb_hit_reg     // 传递给 IDU 做验证
    //什么时候能够发出给下一级的有效呢
    //首先需要本级有效
    //其次要有指令
