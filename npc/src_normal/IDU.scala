...
class IDU extends Module {
    val io = IO(new Bundle {
        ...
        // BTB 信号: 更新 + 误预测 (供性能计数器使用)
        val btb_update      = Output(Bool())
        val btb_mispredict  = Output(Bool())
    ...
    val btb_hit_reg = RegInit(false.B)    // BTB 预测这条指令会跳转
    when(handshake_de) {
        inst_reg := io.in.bits.inst
        pc_reg := io.in.bits.pc
        btb_hit_reg := io.in.bits.btb_hit
    }
    ...
    // ── BTB 预测验证 (IDU 阶段) ──
    // BTB 存条件分支 (B-type) 和 JAL (J-type). hit 意味着"预测这是会跳的分支".
    // 验证:
    //   1. hit + (is_b|is_j) + br_taken → 正确, IFU 已在目标, 无需 flush
    //   2. hit + is_b + !br_taken → mispredict (条件分支不跳), flush 回 pc+4
    //   3. hit + !(is_b|is_j) → 别名冲突 (JALR/非分支):
    //        br_taken → JALR, flush 到 br_target
    //        !br_taken → 非分支, flush 回 pc+4
    val btb_mispredict = btb_hit_reg && !(br_taken && (is_b || is_j))
    val btb_correct    = btb_hit_reg && br_taken && (is_b || is_j)
    val normal_taken   = !btb_hit_reg && br_taken

    io.flush_valid_out := valid && (normal_taken || btb_mispredict || exp_status) && !data_conflict
    // mispredict 且实际不跳 → pc+4; 其余 (normal_taken / JAL别名) → br_target
    io.flush_re_pc := Mux(btb_mispredict && !br_taken, pc_reg + 4.U, br_target)

    // BTB 误预测时阻止错误路径的指令进入 IDU
    io.in.ready := (!valid || (io.out.valid && io.out.ready)) &&
                   !(valid && btb_mispredict && !data_conflict)

    // 输出给性能计数器
    io.btb_mispredict := btb_mispredict && valid && !data_conflict
    ...
    // ── BTB 更新: 存条件分支 (B-type) 和 JAL (J-type) ──
    io.btb_update := idu_fire && br_taken && (is_b || is_j)
