...
extern "C" void dpi_ebreak(int reg_a0) {
  if (g_sim_env) {
    ...
    printf(COLOR_CYAN "[EBREAK] DPI ebreak called with reg_a0 = %d!" COLOR_RESET "\n", reg_a0);
  }
}
...
extern "C" void dpi_commit(int dbg_mstatus, int dbg_mtvec, int dbg_mepc, int dbg_mcause,
    ...
    int last_inst,int next_pc,int last_pc) {
    ...
    if (!g_Difftest->step(npc_regs, last_inst, last_pc)) {
...
extern "C" void mrom_read(int32_t addr, int32_t *data) {
  ...
  uint32_t offset = addr - kMromBase;
  *data = (int32_t)(
    mrom[offset + 0] |
    (mrom[offset + 1] << 8) |
    (mrom[offset + 2] << 16) |
