...
extern int disable_itrace;
extern int disable_mtrace;
...
void log_mtrace_read(uint32_t addr, int data) {
  if (log_fp_mtrace) {
    fprintf(log_fp_mtrace, "[MTRACE] R addr=0x%08x data=0x%08x\n", addr, data);
  }
}

void log_mtrace_write(uint32_t addr, uint32_t data, uint8_t mask) {
  if (log_fp_mtrace) {
    fprintf(log_fp_mtrace, "[MTRACE] W addr=0x%08x data=0x%08x mask=0x%02x\n", addr, data, mask);
  }
}

void log_mtrace_err(const char *msg, uint32_t addr) {
  if (log_fp_mtrace) {
    fprintf(log_fp_mtrace, "[MTRACE] ERROR %s addr=0x%08x out of range\n", msg, addr);
  }
}
